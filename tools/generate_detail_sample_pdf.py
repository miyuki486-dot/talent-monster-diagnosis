...
def paragraph(pdf, text, x, top, width, size=8.2, leading=13, color=INK, bold=False, max_height=120):
    style = ParagraphStyle(
        "jp",
        fontName="NotoJP-Bold" if bold else "NotoJP",
        fontSize=size,
        leading=leading,
        textColor=color,
        alignment=TA_LEFT,
...
def build_pdf(output: Path = OUTPUT):
    ...
    pdf.setFillColor(NAVY)
    pdf.setFont("NotoJP-Bold", 25)
    pdf.drawString(sx, hero_top - 53, "ヒラメキラ")
    pdf.setFillColor(MUTED)
    pdf.setFont("NotoJP-Bold", 9)
    pdf.drawString(sx, hero_top - 70, "ひらめき発明家タイプ")
    paragraph(pdf, "まだないものを思いつく、<br/>アイデアの冒険者", sx, hero_top - 88, sw, size=12, leading=16, color=NAVY, bold=True)
    paragraph(pdf, "決まったやり方にとらわれず、「こんなのはどう？」と新しい景色を描ける子。ふとした思いつきの中に、誰かを驚かせる小さな発明が眠っています。", sx, hero_top - 126, sw, size=8.1, leading=13, color=INK)
    chips = [("発想する力", CYAN), ("工夫する力", YELLOW), ("新しい見方", PINK)]
    chip_x = sx
    for label, color in chips:
        rounded_box(pdf, chip_x, hero_top - 174, 66, 20, color, radius=10)
        pdf.setFillColor(NAVY)
        pdf.setFont("NotoJP-Bold", 7.2)
    ...
    pdf.setFillColor(NAVY)
    pdf.setFont("NotoJP-Bold", 10)
    pdf.drawString(margin + 12, cards_top - 19, "才能を育てるヒント")
    pdf.drawString(margin + card_w + gap + 12, cards_top - 19, "今日からできる最初の一歩")
    paragraph(pdf, "「おもしろいね。もう少し聞かせて」と、まずアイデアを最後まで聞いてみて。実現できるかを急いで決めず、絵やメモにすると才能が育ちます。", margin + 12, cards_top - 30, card_w - 24, size=7.5, leading=11.5, color=INK)
    paragraph(pdf, "思いついたアイデアを絵にして、1枚の「発明メモ」を作ってみよう。", margin + card_w + gap + 12, cards_top - 30, card_w - 24, size=8, leading=12.5, color=INK, bold=True)

    # Evolution story.
    evolution_top = cards_top - card_h - 18
    pdf.setFillColor(NAVY)
    pdf.setFont("NotoJP-Bold", 11)
    pdf.drawString(margin, evolution_top, "モンスターの進化ストーリー")
    pdf.setFillColor(MUTED)
    pdf.setFont("NotoJP", 7.5)
